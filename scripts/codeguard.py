...
def can_embed_inline_index(file_path: str | Path) -> bool:
    force_sidecar = os.getenv("CODEGUARD_FORCE_SIDECAR", "").strip().lower()
    if force_sidecar in {"1", "true", "yes", "on"}:
        return False
    path_obj = Path(file_path)
    sidecar_path = path_obj.with_name(path_obj.name + SIDECAR_INDEX_SUFFIX)
